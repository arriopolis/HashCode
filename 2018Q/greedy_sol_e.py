...
if __name__ == "__main__":
    ...
    for j,(a,b,x,y,s,f) in enumerate(rides):
        ...
        weight = length + 100 * bonus if abs(vx-a) + abs(vy-b) <= s - vt else length
        if time + vt > f:
            print("Job", j, "is not feasible.")
            continue
        job_list.append((weight/time,j,new_score,time+vt))
    ...
    while any(jobs):
        ...
        for j,(a,b,x,y,s,f) in enumerate(rides):
            ...
            jobs[i].append((weight/time,j,new_score,time+vt))
